...
<div class="min-h-dvh bg-[var(--color-bg)] flex items-center justify-center px-4">
  <div class="max-w-sm w-full">
    ...
    <p class="text-gray-400 text-sm mb-6">输入注册邮箱，我们会发送重置链接</p>

    {#if sent}
      <div class="bg-green-500/10 border border-green-500/30 rounded-xl p-6 text-center">
        <div class="text-3xl mb-3">📧</div>
        <p class="text-green-400 text-sm">如果该邮箱已注册，重置链接已发送（开发环境中请查看后端终端日志）</p>
      </div>
    {:else}
      <form onsubmit={handleSubmit} class="space-y-4">
        <div>
          <label for="email" class="block text-sm text-gray-400 mb-1">注册邮箱</label>
